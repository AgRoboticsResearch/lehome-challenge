...
Self-contained: includes inline GarmentRouter (no dependency on train_router.py).
"""
...
# ---------------------------------------------------------------------------
# Inline GarmentRouter (originally from scripts/train_router.py)
# ---------------------------------------------------------------------------
...
class GarmentRouter(nn.Module):
    """Garment type classifier (Router) for MoE expert selection."""

    def __init__(
        self,
        input_dim: int,
        hidden_dims: list = [512, 256, 128],
        num_classes: int = NUM_GARMENT_CLASSES,
        dropout: float = 0.3,
    ):
        super().__init__()
...
class MoESmolVLAPolicy(BasePolicy):
    ...
    def __init__(
        ...
        if model_path == DEFAULT_POLICY_PATH:
            logger.info(f"Ignoring default policy_path: {DEFAULT_POLICY_PATH}")
    ...
    def _get_default_expert_checkpoints(self) -> Dict[str, str]:
        ...
        return {
            "pant_short": str(base_path / "smolvla_moe_expert_pant_short_no_st_proj/checkpoints/011000/pretrained_model"),
            "pant_long": str(base_path / "smolvla_moe_expert_pant_long_no_st_proj/checkpoints/019000/pretrained_model"),
            "top_short": str(base_path / "smolvla_moe_expert_top_short_no_st_proj/checkpoints/020000/pretrained_model"),
            "top_long": str(base_path / "smolvla_moe_expert_top_long_no_st_proj/checkpoints/015000/pretrained_model"),
        }
    ...
    def _log_expert_status(self):
        ...
        for gtype in self.GARMENT_TYPES:
            status = "OK" if gtype in self.available_experts else "MISSING"
            logger.info(f"  [{status}] {gtype}")

        if self.missing_experts:
            logger.warning("-" * 60)
            logger.warning("Some experts are missing!")
    ...
    def _load_expert_preprocessor(self, checkpoint_path: Path):
        ...
        try:
            ...
            logger.warning(f"  Failed to load preprocessor: {e}, using base policy's preprocessor")
            return None
    ...
    def _load_expert_postprocessor(self, checkpoint_path: Path):
        ...
        try:
            ...
            logger.warning(f"  Failed to load postprocessor: {e}, using base policy's postprocessor")
            return None

    def _load_router(self, checkpoint_path: str) -> nn.Module:
        """Load trained router classifier (uses inline GarmentRouter)."""
        checkpoint_path = Path(checkpoint_path)
        ...
        # Reconstruct router using inline GarmentRouter
    ...
    def select_action(self, observation: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # === Sticky Routing: route on first frame and lock ===
        ...
        # === Use locked expert (no more router calls) ===
    ...
    def _route_image(self, image: np.ndarray) -> tuple[str, float]:
        ...
        with torch.no_grad():
            ...
            if garment_type in self.missing_experts:
                logger.warning(f"Router predicted '{garment_type}' but expert not available")
    ...
    def _select_action_with_expert(
        ...
        # Validate that expert has preprocessor
